...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (password_hash) {
      ...
      await sql`UPDATE guru SET nip=${nip}, nama=${nama}, no_wa=${no_wa || null}, jabatan=${jabatan || 'guru'}, password_hash=${hashed} WHERE id=${id}`;
    } else {
      await sql`UPDATE guru SET nip=${nip}, nama=${nama}, no_wa=${no_wa || null}, jabatan=${jabatan || 'guru'} WHERE id=${id}`;
    }
